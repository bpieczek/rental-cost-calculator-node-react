...
function Result(props){
        if(props.values.length !== 0){
            ...
            let timeCostMessage = `Cost of renting a car for one day: ${roundToTwoDecimal(CarRentalCost)} * ${rentalTimeInDays} days = `
            
            CarRentalCost *= rentalTimeInDays;
            timeCostMessage += `${roundToTwoDecimal(CarRentalCost)}`
            
            let carCategoryCostMessage = `Car price category ${props.values[4][0].category}: ${roundToTwoDecimal(CarRentalCost)} + `
            ...
            if(thisYear-props.values[3] < 5){
                newDriversFeeMessage = `Fee for unexperienced drivers: ${roundToTwoDecimal(CarRentalCost)} + 20% = `
                CarRentalCost *= 1.2;
            ...
            if(props.values[4][0].howManyCars < 3){
                lowAvailabilityFee = `Fee for low car availability: ${roundToTwoDecimal(CarRentalCost)} + 15% = `
                CarRentalCost *= 1.15;
            ...
            let combustionCostMessage = `${roundToTwoDecimal(CarRentalCost)} + ${combustionCost} (fuel cost per ${props.values[0]} kilometers) = `; 
            
            CarRentalCost += combustionCost;

            combustionCostMessage += `${roundToTwoDecimal(CarRentalCost)}`;
            let finalCost = `${roundToTwoDecimal(CarRentalCost)} + 23% VAT = ${roundToTwoDecimal(CarRentalCost* 1.23)}`;
